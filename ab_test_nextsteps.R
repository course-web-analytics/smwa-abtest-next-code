...
# ITT Estimation
model_itt <- lm(YOURCODE)
tidy(model_itt) %>%
  mutate(lower_ci = YOURCODE,
         upper_ci = YOURCODE)

# TOT Estimation
ad_data_exposed <- ad_data %>% filter(YOURCODE)

model_tot <- lm(YOURCODE)
tidy(model_tot) %>%
  mutate(lower_ci = YOURCODE,
         upper_ci = YOURCODE)
...
df <-
    df %>%
    mutate(cuped_spend = YOURCODE)

# CUPED: Estimate the ATE
mod_cuped <- lm(YOURCODE)
tidy(mod_cuped)
...
tidy(lm_robust(YOURCODE,
               se_type = "HC1"), conf.int = FALSE
     )

# Cluster Robust Standard Errors
tidy(lm_robust(YOURCODE,
               cluster = YOURCODE), conf.int = FALSE
     )
